...
class EnhancedEmbeddingGenerator:
    ...
    def _ensure_document_format(self, chunk: Union[str, dict, Document]) -> Document:
        """
        Ensure the chunk is in Document format
        
        Args:
            chunk: Input chunk that could be a string, dict, or Document
            
        Returns:
            Document: Properly formatted Document object
        """
        if isinstance(chunk, str):
            return Document(page_content=chunk)
        elif isinstance(chunk, dict) and "page_content" in chunk:
            content = chunk["page_content"]
            metadata = {k: v for k, v in chunk.items() if k != "page_content"}
            return Document(page_content=content, metadata=metadata)
        elif isinstance(chunk, Document):
            return chunk
        else:
            raise ValueError(f"Unsupported chunk format: {type(chunk)}. Expected str, dict with 'page_content', or Document.")
        
    def generate_embeddings(self, chunks: List[Union[str, dict, Document]]) -> List[List[float]]:
        """
        Generate embeddings with batching and caching
        
        Args:
            chunks: List of chunks (can be strings, dicts with 'page_content', or Document objects)
            
        Returns:
            List[List[float]]: Generated embeddings
        """
        # Convert all chunks to Document format
        formatted_chunks = []
        for chunk in chunks:
            try:
                formatted_chunks.append(self._ensure_document_format(chunk))
        ...
        for i, chunk in enumerate(formatted_chunks):
            ...
            if content in self.embedding_cache:
                ...
                if len(batch) >= self.batch_size:
                    ...
                    # Insert the embeddings at the correct positions
