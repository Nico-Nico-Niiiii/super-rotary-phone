#!/usr/bin/env python3
"""
All-in-one RAG system with LangChain and LangGraph
Includes document processing, Azure OpenAI integration, and interactive querying

Usage:
python rag_complete_fixed.py --zip-path "path/to/your/documents.zip" --rag-type raptor
"""
...
class LangGraphRAG:
    ...
    def _create_standard_rag_workflow(self):
        ...
        def generate(state: State) -> State:
            ...
            # Use a simpler PromptTemplate instead of ChatPromptTemplate to avoid template issues
    ...
    def _create_iterative_rag_workflow(self):
        ...
        def generate(state: State) -> State:
            ...
            return {
                **state,
                "answer": answer,
                "follow_up_needed": iteration < 3  # Allow up to 3 iterations
            }
    ...
    def _create_self_reflective_rag_workflow(self):
        ...
        def generate_final(state: State) -> State:
            ...
            if relevance_score >= 0.8:
                # Draft answer is good enough
                return {**state, "answer": draft_answer}
        ...
        builder.add_edge("evaluate_relevance", "generate_final")
    ...
    def _create_adaptive_rag_workflow(self):
        """Create an adaptive RAG workflow that adjusts its search strategy based on initial results"""
        # Define the state
        class State(TypedDict):
            query: str
            search_strategy: str
...
def main():
    """Main function - process arguments and run RAG system"""
    parser = argparse.ArgumentParser(description="All-in-one RAG System with Azure OpenAI")
    parser.add_argument("--zip-path", type=str, required=True, help="Path to the documents zip file")
    parser.add_argument("--rag-type", type=str, default="raptor", 
    ...
    try:
        ...
        print(f"Initializing {args.rag_type} RAG system...")
        rag = LangGraphRAG(
        ...
        # Process documents
        print(f"Processing documents from {args.zip_path}...")
        rag.process_documents(args.zip_path)
        ...
        while True:
            ...
            try:
                ...
                logger.error(f"Error generating response: {e}", exc_info=True if args.verbose else False)
